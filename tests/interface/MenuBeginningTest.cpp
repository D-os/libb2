...
void show_window(BWindow *window)
{
	...
	menu->AddItem(new BMenuItem("TWO", new BMessage('1TWO')));
	bar->AddItem(menu);

	menu = new BMenu("menu TWO");
	menu->AddItem(new BMenuItem("ONE", new BMessage('2ONE')));
...
void TestWindow::MenusBeginning()
{
	printf("MenusBeginning()\n");
}

void TestWindow::MenusEnded()
{
	printf("MenusEnded()\n");
}
