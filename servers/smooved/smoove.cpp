...
#if 0
void start_package_manager(const SContext& context)
{
...
class ProcessManager : public BCatalog, public BnProcessManager
{
 public:
  ProcessManager(const SContext& context)
      : BCatalog(context), BnProcessManager(context), m_lock("ProcessManager:m_lock")
  {
  }
...
void start_process_manager(const SContext& context)
{
...
int main(int argc, char* argv[])
{
  ...
  if (!SLooper::BecomeContextManager(validate_context, NULL)) {
    fprintf(stderr, "smooved FAILED to register the context manager, exiting...\n");
    return B_ERROR;
  }
