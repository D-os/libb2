...
/*!	The main purpose of this class is to deal with the mechanics of walking
	...
	@nosubgrouping
*/
class BGenericNode : public support::BnNode
{
 public:
  // --------------------------------------------------------------
  /*!	@name Bookkeeping
  ...
  //@{
  BGenericNode();
  BGenericNode(const support::SContext& context);
 ...
 public:
  ...
  virtual support::sptr<INode> Attributes() const;
  //!	Acquires the node lock and calls MimeTypeLocked().
  virtual support::SString MimeType() const;
  //!	Acquires the node lock and calls SetMimeTypeLocked().
  virtual void SetMimeType(const support::SString& value);
  ...
  virtual status_t Walk(support::SString* path, uint32_t flags, support::SValue* node);

  //!	Calls StoreMimeTypeLocked() and updates datums, pushes event.
  status_t SetMimeTypeLocked(const support::SString& value);
  ...
  virtual status_t LookupEntry(const support::SString& entry, uint32_t flags, support::SValue* node) = 0;

  //!	Called as part of walking with the CREATE_NODE flag set.
  /*!	The default implementation simply returns NULL. */
  virtual support::sptr<INode> CreateNode(support::SString* name, status_t* err);

  //!	Called as part of walking with the CREATE_DATUM flag set.
  /*!	The default implementation simply returns NULL. */
  virtual support::sptr<support::IDatum> CreateDatum(support::SString* name, uint32_t flags, status_t* err);
  ...
  virtual status_t LookupMetaEntry(const support::SString& entry, uint32_t flags, support::SValue* node);
  ...
  virtual status_t CreateMetaEntry(const support::SString& name, const support::SValue& initialValue, support::sptr<support::IDatum>* outDatum = NULL);
  ...
  virtual status_t RemoveMetaEntry(const support::SString& name);

  //!	Change the name of a piece of meta-data in the node.
  /*!	The default implementation returns B_UNSUPPORTED to indicate that it
				can not rename meta-data. */
  virtual status_t RenameMetaEntry(const support::SString& old_name, const support::SString& new_name);
  ...
  virtual status_t MetaEntryAtLocked(ssize_t index, uint32_t flags, support::SValue* key, support::SValue* entry);
 ...
 private:
  ...
  support::sptr<node_state> get_state_l() const;
  status_t                  get_meta_value_l(ssize_t index, support::SValue* key, support::SValue* value);
  status_t                  datum_changed_l(size_t index, const support::SValue& newValue);

  mutable support::SLocker          m_lock;
  mutable support::wptr<node_state> m_state;
};
