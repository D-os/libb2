...
inline BSize::BSize(const BSize& other)
	: width(other.width),
	  height(other.height)
{
}
...
inline float BSize::Width() const
{
	return width;
}

inline float BSize::Height() const
{
	return height;
}
...
{
	return width != float(B_SIZE_UNSET);
}

inline bool BSize::IsHeightSet() const
{
	return height != float(B_SIZE_UNSET);
}
...
inline BSize& BSize::operator=(const BSize& other)
{
	width  = other.width;
	height = other.height;
	return *this;
