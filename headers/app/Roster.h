...
class BRoster
{
   public:
	...
	status_t Launch(const char *mime_type, BMessage *initial_msgs = nullptr,
					team_id *app_team = nullptr) const;
	status_t Launch(const char *mime_type, BList *message_list,
					team_id *app_team = nullptr) const;
	status_t Launch(const char *mime_type, int argc, char **args,
					team_id *app_team = nullptr) const;

	status_t Launch(const entry_ref *ref, const BMessage *initial_message = nullptr,
					team_id *app_team = nullptr) const;
	...
	void GetRecentDocuments(BMessage *refList, int32 maxCount,
							const char *ofType		   = nullptr,
							const char *openedByAppSig = nullptr) const;
	void GetRecentDocuments(BMessage *refList, int32 maxCount,
	...
	void AddToRecentDocuments(const entry_ref *doc,
							  const char	  *appSig = nullptr) const;
   ...
   private:
	...
	uint32 _AddApplication(const char *mime_sig,
						   entry_ref  *ref,
						   uint32	   flags,
						   team_id	   team,
						   thread_id   thread,
	...
	// status_t _LaunchApp(const char		*mime_sig,
	// 					const entry_ref *ref,
	// 					BList			*msg_list,
	// 					int				 cargs,
	// 					char		   **args,
