...
#define DEBUGGER(MSG) \
  if (_rtDebugFlag) ErrFatalError(MSG);
#if !defined(ASSERT)
...
#define TRESPASS()                                                                                                  \
  if (1) {                                                                                                          \
    _debugPrintf("Should not be here at File: %s, Line: %d, Thread: %d\n", __FILE__, __LINE__, SysCurrentThread()); \
...
template <class STATE>
atomic_int BDebugState<STATE>::m_hasState(0);
template <class STATE>
STATE* BDebugState<STATE>::m_state = NULL;
