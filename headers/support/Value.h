...
Value method such as SValue::Int32(), but semantically
they are equivalent.
...
/*!	Value is formally an abstract set of <tt>{key->value}</tt> mappings, where
...
Value that is not set; \c B_WILD_VALUE is a special value that can be thought of
...
Note that the definition of Value is very recursive -- it is a
collection of key/value mappings, where both the key and value are
themselves a Value.  As such, a Value containing a single data
...
so that a set of the values <tt>{ A, B }</tt> can be constructed in a Value
...
the mapping <tt>{WILD->data}</tt>. */
    ...
    SValue(type_code type, const SharedBuffer* buffer);
    ...
    SValue(const sp<IBinder>& binder);
    SValue(const wp<IBinder>& binder);
    //@}
...
Assignment, status, existence, etc. */
    ...
    */
    ...
    /*!	Error values are special, and consume any other value
        ...
        inside of Value itself.  Users will normally want
...
<tt>{WILD->data}</tt>. */
    ...
    size_t			length() const;
    //!	Return raw value data as a SharedBuffer.
    const SharedBuffer*	sharedBuffer() const;

    //! Make sure value's data is in the SharedBuffer pool.
    void			pool();
...
Working with an Value as structured data. */
    //@{
...
@endcode
    ...
    */
    ...
    /*!	This is used in conjunction with ReplaceValues() to efficiently change
        a set of items in an Value mapping.  This function is called first, filling
...
Reading and writing as flat byte streams. */
    ...
    ssize_t			archive(Parcel& into) const;
//    ssize_t			Archive(const sp<IByteOutput>& into) const;
    ssize_t			unarchive(Parcel& from);
//    ssize_t			Unarchive(const sp<IByteInput>& from);

    //@}

    /*!	@name Comparison
The common Value comparison operation is a quick binary
...
a conversion. */
    ...
    */
    sp<IBinder>	asBinder(status_t* result = NULL) const;
    ...
    /*!	Converts from the following types:
        ...
        to the current error/status code or B_BAD_TYPE.
    */
    wp<IBinder>	asWeakBinder(status_t* result = NULL) const;
    ...
    */
    ...
    /*!	Converts from the following types:
            ...
            - B_REFBASE_TYPE: Converted to true if address is not NULL.
            - B_REFBASE_WEAK_TYPE: Converted to true if address is not NULL.
    ...
    */
    ...
    /*!	Converts from the following types:
        ...
        to the current error/status code or B_BAD_TYPE.
    */
    int			asInt(status_t* result = NULL) const;
    ...
    */
    ...
    //!	Convert value to a strong RefBase pointer.
    /*!	Converts from the following types:
            - B_REFBASE_TYPE: Returned as-is.
            - B_REFBASE_WEAK_TYPE: Attempts to promote to strong pointer.
            - B_NULL_TYPE: Returns NULL.
        For all other types, returns NULL and 'result' is set
        to the current error/status code or B_BAD_TYPE.
    */
    sp<RefBase>		asRefBase(status_t* result = NULL) const;

    //!	Convert value to a weak RefBase pointer.
    /*!	Converts from the following types:
            - B_REFBASE_TYPE: Returned as a weak pointer.
            - B_REFBASE_WEAK_TYPE: Returns as-is.
            - B_NULL_TYPE: Returns NULL.
        For all other types, returns NULL and 'result' is set
        to the current error/status code or B_BAD_TYPE.
    */
    wp<RefBase>		asWeakRefBase(status_t* result = NULL) const;

    //!	Synonym for AsInt64().
    inline	off_t			asOffset(status_t* result = NULL) const;
...
(These functions fail if the value is not exactly the requested type.) */
    ...
    status_t		getBinder(sp<IBinder> *obj) const;
    status_t		getWeakBinder(wp<IBinder> *obj) const;
    ...
    status_t		getRefBase(sp<RefBase>* refbase) const;
    status_t		getWeakRefBase(wp<RefBase>* refbase) const;
...
Function for printing values in a nice format. */
    ...
    status_t		printToStream(const TextOutput& io, uint32_t flags = 0) const;

    //!	Value pretty printer function.
    /*!	\param	io		The stream to print in.
        \param	val		The value to print.
        \param	flags	Additional formatting information.
    */
    typedef	status_t			(*print_func)(	const TextOutput& io,
                                                const SValue& val,
                                                uint32_t flags);
...
private:
    ...
    void			init_as_shared_buffer(type_code type, const SharedBuffer* buffer);
    ...
    ssize_t			unarchive_internal(Parcel& from, size_t amount);
    BValueMap**		edit_map();
    ...
    union {
        // out-of-line storage
        const SharedBuffer*	buffer;		// B_TYPE_LENGTH_BIG -- block of data
...
Some higher level TypeFuncs that need to know about Value
BArrayAValue & BArrayConstruct
...
of the type (called by PerformAValue and PerformSetFromValue).
...
Value TYPE::AValue() and TYPE::TYPE(const SValue&)	: B_IMPLEMENT_FLATTEN_FUNCS()
...
Please note that sequences do not support SVector< wp<TYPE> >.
*/
//@{
...
extern SharedBuffer* BArrayAValueHelper(void*& toPtr, size_t count, size_t elementSize, type_code typeCode);
...
inline SValue BArrayAValue(const TYPE* from, size_t count)									\
{																							\
void* voidPtr = NULL;																	\
SharedBuffer* buffer = BArrayAValueHelper(voidPtr, count, sizeof(TYPE), TYPECODE);	\
...
result.joinItem(SSimpleValue<int32_t>(i), from->AValue());				\
from++;																	\
}																			\
return result;																\
}																				\
...
// Value TYPE::AValue() and TYPE::TYPE(const SValue&)
// -----------------------------------------------------------------
...
// -----------------------------------------------------------------
// Specializations for sp<IInterface>
// -----------------------------------------------------------------

#define B_IMPLEMENT_IINTERFACE_FLATTEN_FUNCS(TYPE)											\
inline SValue BArrayAValue(const sp<TYPE>* from, size_t count)						\
{																						\
SValue result;																		\
for (size_t i = 0; i < count; i++) {												\
...
inline status_t BArrayConstruct(sp<TYPE>* to, const SValue& value, size_t count)		\
{																						\
BConstruct(to, count);																\
for (size_t i = 0; i < count; i++) {												\
...
// -----------------------------------------------------------------
// Specializations for sp<IBinder>
// -----------------------------------------------------------------

inline SValue BArrayAValue(const sp<IBinder>* from, size_t count)
{
    SValue result;
    for (size_t i = 0; i < count; i++) {
        result.joinItem(SSimpleValue<int32_t>(i), SValue(*from));
        from++;
    }
    return result;
}

inline status_t BArrayConstruct(sp<IBinder>* to, const SValue& value, size_t count)
{
    BConstruct(to, count);
    for (size_t i = 0; i < count; i++) {
...
// -----------------------------------------------------------------
// Specializations for marshalling SVector<Value>
// -----------------------------------------------------------------

SValue BArrayAValue(const SValue* from, size_t count);
...
// that we can never change the fact that an undefined Value contains
...
{
    //printf("*** Creating Value %p from Value\n", this);
    initAsCopy(o);
}

inline SValue::SValue(const SValue& key, const SValue& value)
{
    //printf("*** Creating Value %p from Value->Value\n", this);
    initAsMap(key, value);
}
...
{
    //printf("*** Creating Value %p from type,data,len\n", this);
