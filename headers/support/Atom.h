...
class SLightAtom
{
 ...
 private:
  ...
  union
  {
    ...
    mutable BNS(os::osp::) atom_debug* m_debugPtr;
  };
...
class SLimAtom
{
 public:
  ...
  inline void DecStrongFast() const
  {
    if (atomic_fetch_dec(&m_strongCount) == 1) { delete static_cast<const T*>(this); }
  }
...
#define COMPARE(op)                                               \
  template <class TYPE>                                           \
  inline bool sptr<TYPE>::operator op(const TYPE* p2) const       \
  {                                                               \
    return ptr() op p2;                                           \
  }                                                               \
  template <class TYPE>                                           \
  inline bool sptr<TYPE>::operator op(const sptr<TYPE>& p2) const \
  {                                                               \
    return ptr() op p2.ptr();                                     \
  }                                                               \
  template <class TYPE>                                           \
  inline bool sptr<TYPE>::operator op(const wptr<TYPE>& p2) const \
  {                                                               \
    return ptr() op p2.unsafe_ptr_access();                       \
  }                                                               \
  template <class TYPE>                                           \
  inline bool wptr<TYPE>::operator op(const TYPE* p2) const       \
  {                                                               \
    return unsafe_ptr_access() op p2;                             \
  }                                                               \
  template <class TYPE>                                           \
  inline bool wptr<TYPE>::operator op(const sptr<TYPE>& p2) const \
  {                                                               \
    return unsafe_ptr_access() op p2.ptr();                       \
  }                                                               \
  template <class TYPE>                                           \
  inline bool wptr<TYPE>::operator op(const wptr<TYPE>& p2) const \
  {                                                               \
...
template <class TYPE>
inline sptr<TYPE>::sptr()
{
  m_ptr = NULL;
}
template <class TYPE>
inline sptr<TYPE>::sptr(TYPE* p)
{
  if ((m_ptr = p) != NULL) B_INC_STRONG(p, this);
}

template <class TYPE>
inline sptr<TYPE>& sptr<TYPE>::operator=(TYPE* p)
{
...
template <class TYPE>
inline sptr<TYPE>::sptr(const sptr<TYPE>& p)
{
  if ((m_ptr = p.m_ptr) != NULL) B_INC_STRONG(m_ptr, this);
}
template <class TYPE>
inline sptr<TYPE>& sptr<TYPE>::operator=(const sptr<TYPE>& p)
{
...
template <class TYPE>
template <class NEWTYPE>
inline sptr<TYPE>::sptr(const sptr<NEWTYPE>& p)
{
...
template <class TYPE>
template <class NEWTYPE>
inline sptr<TYPE>& sptr<TYPE>::operator=(const sptr<NEWTYPE>& p)
{
  TypeConversion<NEWTYPE, TYPE>::Assert();
  if (p.ptr()) B_INC_STRONG(p, this);
  if (m_ptr) B_DEC_STRONG(m_ptr, this);
...
template <class TYPE>
inline sptr<TYPE>::sptr(SAtom::weak_atom_ptr* p, bool)
{
...
template <class TYPE>
inline TYPE& sptr<TYPE>::operator*() const
{
  return *m_ptr;
}
template <class TYPE>
...
template <class TYPE>
inline TYPE* sptr<TYPE>::ptr() const
{
  return m_ptr;
}
template <class TYPE>
inline TYPE* sptr<TYPE>::detach()
{
  TYPE* p = m_ptr;
  m_ptr   = NULL;
  return p;
...
template <class TYPE>
inline bool sptr<TYPE>::is_null() const
{
  return m_ptr == NULL;
}

template <class TYPE>
inline TYPE* sptr<TYPE>::edit()
{
  TYPE* p = m_ptr;
  if (p != NULL) {
    p     = p->Edit();
...
template <class TYPE>
inline TYPE* sptr<TYPE>::edit(size_t size)
{
  TYPE* p = m_ptr;
  if (p != NULL) {
    p     = p->Edit(size);
...
template <class TYPE>
inline wptr<TYPE>::wptr()
{
  m_ptr = NULL;
}

template <class TYPE>
inline wptr<TYPE>::wptr(TYPE* p)
{
  if (p) {
...
template <class TYPE>
inline wptr<TYPE>& wptr<TYPE>::operator=(TYPE* p)
{
...
template <class TYPE>
inline SAtom::weak_atom_ptr* wptr<TYPE>::get_weak_atom_ptr() const
{
  return m_ptr;
}

template <class TYPE>
inline void wptr<TYPE>::set_weak_atom_ptr(SAtom::weak_atom_ptr* weak)
{
  weak->Increment(this);
...
template <class TYPE>
inline wptr<TYPE>::wptr(const wptr<TYPE>& p)
{
  if ((m_ptr = p.m_ptr) != NULL) m_ptr->Increment(this);
}

template <class TYPE>
inline wptr<TYPE>::wptr(const sptr<TYPE>& p)
{
  if (p.ptr()) {
...
template <class TYPE>
inline wptr<TYPE>& wptr<TYPE>::operator=(const wptr<TYPE>& p)
{
...
template <class TYPE>
inline wptr<TYPE>& wptr<TYPE>::operator=(const sptr<TYPE>& p)
{
  SAtom::weak_atom_ptr* weak = NULL;
  if (p.ptr()) {
...
template <class TYPE>
template <class NEWTYPE>
inline wptr<TYPE>::wptr(const wptr<NEWTYPE>& p)
{
  TypeConversion<NEWTYPE, TYPE>::Assert();
  if (p.m_ptr) {
...
template <class TYPE>
template <class NEWTYPE>
inline wptr<TYPE>::wptr(const sptr<NEWTYPE>& p)
{
  TypeConversion<NEWTYPE, TYPE>::Assert();
  if (p.ptr()) {
...
template <class TYPE>
template <class NEWTYPE>
inline wptr<TYPE>& wptr<TYPE>::operator=(const sptr<NEWTYPE>& p)
{
  TypeConversion<NEWTYPE, TYPE>::Assert();
  SAtom::weak_atom_ptr* weak = NULL;
  if (p.ptr()) {
...
template <class TYPE>
template <class NEWTYPE>
inline wptr<TYPE>& wptr<TYPE>::operator=(const wptr<NEWTYPE>& p)
{
  TypeConversion<NEWTYPE, TYPE>::Assert();
  SAtom::weak_atom_ptr* weak = NULL;
  if (p.m_ptr) {
...
template <class TYPE>
inline TYPE* wptr<TYPE>::unsafe_ptr_access() const
{
  return (m_ptr != NULL) ? reinterpret_cast<TYPE*>(m_ptr->cookie) : NULL;
}

template <class TYPE>
inline const sptr<TYPE> wptr<TYPE>::promote() const
{
  return sptr<TYPE>(m_ptr, true);
}

template <class TYPE>
inline void wptr<TYPE>::inc_weak(const void* id) const
{
  if (m_ptr) B_INC_WEAK(m_ptr->atom, id);
}

template <class TYPE>
inline void wptr<TYPE>::dec_weak(const void* id) const
{
