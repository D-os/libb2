...
BSchemaTableNode::BSchemaTableNode(const SContext& context, const SDatabase& database,
    ...
    : BMetaDataNode(context), BGenericIterable(context), BnCatalog(context), BnTable(context), BnReferable(context), m_database(database), m_table(table), m_keyColumn(keyColumn), m_refPath(refPath), m_rowMimeType(MimeTypeLocked()), m_columnNameToIndex(B_ENTRY_NOT_FOUND), m_accessorsLock("BSchemaTableNode::m_accessorsLock")
{
...
BSchemaTableNode::DataAccessor::DataAccessor(const sptr<BSchemaTableNode>& table)
    : m_table(table), m_knowNumColumns(false), m_numDepColumns(0), m_numBaseColumns(0), m_numCustomColumns(0)
{
}
  ...
  if (ND + NB > 0) {
    ...
    if (err != B_OK) {
      ...
      for (i = 0; i < ND + NB; i++) {
        ...
        if (cd.errCode == dmErrNoColumnData)
          v = B_NULL_VALUE;
        else
          v = SValue::Status(cd.errCode);
      }
    }
  }
...
status_t BSchemaTableNode::DataAccessor::prep_column_read_l(uint32_t row, const column_info& ci, uint32_t flags, DbSchemaColumnValueType* outData, SValue* intoValue, size_t maxDataSize) const
{
  ...
  if (ci.varsize) {
    ...
    if (err != B_OK) {
      if (err == dmErrNoColumnData)
        *intoValue = B_NULL_VALUE;
      else
        *intoValue = SValue::Status(err);
      return B_OK;
    }
...
BSchemaTableNode::RowNode::RowNode(const SContext& context, const sptr<BSchemaTableNode>& owner, uint32_t rowID)
    : BIndexedDataNode(context), DataAccessor(owner), m_rowID(rowID)
{
}
...
BSchemaTableNode::QueryIterator::QueryIterator(const SContext& context, const sptr<BSchemaTableNode>& owner)
    : GenericIterator(context, owner.ptr()), DataAccessor(owner), m_ref(owner->m_ref), m_status(B_OK), m_cursor(dbInvalidCursorID)
{
}
