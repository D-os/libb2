...
BXML2ValueCreator::BXML2ValueCreator(SValue &targetValue, const SValueMap &attributes, const SPackage &resources,
                                     const SValueMap &references)
    : m_resources(resources), m_targetValue(targetValue), m_status(B_OK), m_references(references), m_isReference(false)
{
  m_id = gID++;
  ...
  if (key.IsDefined()) {
    ...
    err = attributes[kIDTypeAttribute].getString(&key_type);
    if (err != B_OK)
      key_type = "string";
...
#if !LINUX_DEMO_HACK
    if (m_resources == B_NO_PACKAGE)
      m_value = SValue::Status(B_BAD_DATA);
    else
      m_value = SValue::String(m_resources.LoadString(m_resID, m_strIndex));
#endif
  }
