...
autobinder_to_parcel(const BEffectMethodDef& def, void** args, void* result, SParcel& parcel, uint32_t dirs)
{
  ...
  STUB; /*if (def.returnType != B_UNDEFINED_TYPE || hasOut) {
                   ...
                   uint32_t flags)
{
  ...
  if (index >= def_count) {
    ...
    STUB;  //data.print(bout);
    bout << endl
         << "execute_autobinder reply=" << (reply ? reply->data() : NULL) << " ";
    if (reply) {
      STUB;  //reply->print(bout);
    }
    else {
      bout << "NULL";
    }
...
parameter_to_value(type_code type, const struct PTypeMarshaller* marshaller, const void* value, SValue* out)
{
  ...
  switch (type) {
    case B_BOOL_TYPE:
      *out = SValue(*reinterpret_cast<const bool*>(value));

    case B_FLOAT_TYPE:
      *out = SValue(*reinterpret_cast<const float*>(value));
      break;

    case B_DOUBLE_TYPE:
      *out = SValue(*reinterpret_cast<const double*>(value));
      break;

    case B_CHAR_TYPE:
    case B_INT8_TYPE:
    case B_UINT8_TYPE:
      *out = SValue(*reinterpret_cast<const int8_t*>(value));
      break;

    case B_INT16_TYPE:
    case B_UINT16_TYPE:
      *out = SValue(*reinterpret_cast<const int16_t*>(value));
      break;
    ...
    case B_SSIZE_T_TYPE:
    case B_WCHAR_TYPE:
      *out = SValue(*reinterpret_cast<const int32_t*>(value));
      break;
    ...
    case B_BIGTIME_TYPE:
    case B_OFF_T_TYPE:
      *out = SValue(*reinterpret_cast<const int64_t*>(value));
      break;

    case B_BINDER_TYPE:
      STUB;  //*out = SValue::Binder(*reinterpret_cast<const sptr<IBinder>*>(value));
      break;

    case B_BINDER_WEAK_TYPE:
      STUB;  //*out = SValue::WeakBinder(*reinterpret_cast<const wptr<IBinder>*>(value));
      break;
    ...
    case B_CONSTCHAR_TYPE:
    case B_STRING_TYPE:
      *out = SValue(*reinterpret_cast<const SString*>(value));
      break;
...
parameter_from_value(type_code type, const struct PTypeMarshaller* marshaller, const SValue& v, void* result)
{
  ...
  switch (type) {
    case B_BOOL_TYPE:
      v.getBoolean(reinterpret_cast<bool*>(result));

    case B_FLOAT_TYPE:
      STUB;  //*reinterpret_cast<float*>(result) = v.AsFloat();
      break;

    case B_DOUBLE_TYPE:
      v.getDouble(reinterpret_cast<double*>(result));
      break;

    case B_CHAR_TYPE:
    case B_INT8_TYPE:
    case B_UINT8_TYPE:
      v.getByte(reinterpret_cast<int8_t*>(result));
      break;

    case B_INT16_TYPE:
    case B_UINT16_TYPE:
      STUB;  //*reinterpret_cast<int16_t*>(result) = static_cast<int8_t>(v.AsInt32());
      break;
    ...
    case B_SSIZE_T_TYPE:
    case B_WCHAR_TYPE:
      v.getInt(reinterpret_cast<int32_t*>(result));
      break;

    case B_INT64_TYPE:
      v.getLong(reinterpret_cast<int64_t*>(result));
      break;

    case B_BINDER_TYPE:
      STUB;  //*reinterpret_cast<sptr<IBinder>*>(result) = v.AsBinder();
      break;

    case B_BINDER_WEAK_TYPE:
      STUB;  //*reinterpret_cast<wptr<IBinder>*>(result) = v.AsWeakBinder();
      break;

    case B_VALUE_TYPE:
      STUB;  //*reinterpret_cast<SValue*>(result) = v;
      break;
#if 0
		case B_CONSTCHAR_TYPE:
    ...
    case B_CONSTCHAR_TYPE:
    case B_STRING_TYPE:
      v.getString(reinterpret_cast<SString*>(result));
      break;
