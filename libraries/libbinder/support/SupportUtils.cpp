...
void rename_object(const small_flat_data& obj, const void* newWho, const void* oldWho)
{
  ...
  if (atom)
    atom->RenameOwnerID(newWho, oldWho);
  else*/
  if (!matched)
    DbgOnlyFatalError("Bad type supplied to rename_object()");
}
#endif
  ...
  if (binder != NULL) {
    ...
    if (real != NULL) {
      ...
      if (!local) {
        ...
        STUB;  //out->data.object = binder.get_weak_atom_ptr();
      }
    }
    else {
      ...
      STUB; /*SAtom::weak_atom_ptr* weak  = binder.get_weak_atom_ptr();
...
#if !LIBBE_BOOTSTRAP
    case kPackedSmallBinderHandleType:
      *out = ProcessState::self()->getStrongProxyForHandle(flat.data.integer);
      return B_OK;
#endif
  }
  return B_BAD_TYPE;
...
status_t unflatten_binder(const small_flat_data& flat, wptr<IBinder>* out)
{
  switch (flat.type) {
    ...
    case kPackedSmallBinderWeakType: {
      STUB;  //SAtom::weak_atom_ptr* weak = static_cast<SAtom::weak_atom_ptr*>(flat.data.object);
    ...
    case kPackedSmallBinderHandleType:
    case kPackedSmallBinderWeakHandleType:
      *out = ProcessState::self()->getWeakProxyForHandle(flat.data.integer);
      return B_OK;
#endif
  }
  return B_BAD_TYPE;
...
#if !LIBBE_BOOTSTRAP
    case kPackedLargeBinderHandleType:
      *out = ProcessState::self()->getStrongProxyForHandle(flat.handle);
      return B_OK;
#endif
  }*/
  return B_BAD_TYPE;
...
status_t unflatten_binder(const flat_binder_object& flat, wptr<IBinder>* out)
{
  ...
  /*switch (flat.type) {
    ...
    case kPackedLargeBinderHandleType:
    case kPackedLargeBinderWeakHandleType:
      *out = ProcessState::self()->getWeakProxyForHandle(flat.handle);
      return B_OK;
#endif
  }*/
  return B_BAD_TYPE;
