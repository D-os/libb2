...
BTextOutput::style_state::style_state()
    : tag(0), indent(0), front(1), buffering(false), buffer(NULL), bufferLen(0), bufferAvail(0)
{
}
...
BTextOutput::BTextOutput(const sptr<IByteOutput>& stream, uint32_t flags)
    : m_stream(stream.get()), m_flags(flags), m_threadStyles(NULL), m_nextTag(0)
{
#if SUPPORTS_TEXT_STREAM
  InitStyles();
#endif
  m_stream->incStrong(this);
}
...
{
  STUB;
  return 0;  //m_stream->asBinder();
}

sptr<const IBinder> BTextOutput::AsBinderImpl() const
{
  STUB;
  return 0;  //((const IByteOutput*)m_stream)->asBinder();
}
  ...
  if ((m_flags & B_TEXT_OUTPUT_FROM_ENV) != 0) {
    ...
    while (env && *env) {
      switch (*env) {
        ...
        case 'c':
          if (enable)
            m_flags |= B_TEXT_OUTPUT_COLORED | defs;
          else
            m_flags &= ~B_TEXT_OUTPUT_COLORED;
          enable = true;
          if (env[1] >= '0' && env[1] <= '9') {
            m_numColors = 0;
            m_colors[0] = 0;
            ...
            } while ((env[1] >= '0' && env[1] <= '9') || (env[1] == ',' && m_numColors < (MAX_COLORS - 1)));
            m_numColors++;
          }
          break;
        case 'p':
          if (enable)
            m_flags |= B_TEXT_OUTPUT_TAG_TEAM | defs;
          else
            m_flags &= ~B_TEXT_OUTPUT_TAG_TEAM;
          enable = true;
          break;
        case 't':
          if (enable)
            m_flags |= B_TEXT_OUTPUT_TAG_THREAD | defs;
          else
            m_flags &= ~B_TEXT_OUTPUT_TAG_THREAD;
          enable = true;
          break;
        case 'w':
          if (enable)
            m_flags |= B_TEXT_OUTPUT_TAG_TIME | defs;
          else
            m_flags &= ~B_TEXT_OUTPUT_TAG_TIME;
          enable = true;
          break;
        case '!':
          enable = false;
...
BTextInput::BTextInput(const sptr<IByteInput>& stream, uint32_t flags)
    : m_stream(stream.get()), m_flags(flags), m_lock("BTestInput"), m_pos(0), m_avail(0), m_inCR(false)
{
  m_stream->incStrong(this);
}

BTextInput::BTextInput(IByteInput* This, uint32_t flags)
    : m_stream(This), m_flags(flags), m_lock("BTextInput"), m_pos(0), m_avail(0), m_inCR(false)
{
}
...
{
  STUB;
  return 0;  //((const IByteInput*)m_stream)->asBinder();
}
...
SHexDump::SHexDump(const void* buf, size_t length, size_t bytesPerLine)
    : fBuffer(buf), fLength(length), fBytesPerLine(bytesPerLine), fSingleLineCutoff(bytesPerLine), fAlignment(4), fCArrayStyle(false)
{
  if (bytesPerLine >= 16)
    fAlignment = 4;
