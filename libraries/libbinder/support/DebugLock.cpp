...
struct link_info
{
  ...
  inline link_info(const link_info& o)
      : target(o.target), source(o.source), thread(o.thread), stack(o.stack), count(o.count)
  {
  }
...
static inline void AddLockHeld(DebugLockNode* lock)
{
  ...
  if (!held) {
    held = new (std::nothrow) SSortedVector<DebugLockNode*>;
    tls_set(gLocksHeldTLS, held);
  }
  if (held) held->add(lock);
}
                                                 ...
                                                 DebugLockNode* base)
{
  if (base->Links()) {
    ...
    int32_t                    i = base->Links()->sources.size();
    while (i > 0) {
      --i;
      DebugLockNode* s = base->Links()->sources.itemAt(i);
      if (s == source) {
        v = new (std::nothrow) SVector<const link_info*>;
      ...
      if (v) {
        v->add(&(s->Links()->targets.ValueFor(base)));
        if (!shortest) {
          shortest = v;
        }
        else if (shortest->size() > v->size()) {
          delete shortest;
          shortest = v;
        }
        else {
...
static bool RegisterDebugNode(SVector<DebugLockNode*>& vec, DebugLockNode* who, get_node_value_func valGetter, remove_node_func nodeRemover)
{
  ...
  if (low < N) {
    ...
    vec.insertAt(who, low);
    //printf("Adding contention at %ld: %p (now have %ld)", low, who, gHighContention->size());
    return true;
  }
  return false;
}
...
static void AllocContentionLock()
{
  if (atomic_fetch_or(&gHasContentionSem, 1) == 0) {
    ...
    if (gNumContention > 0) {
      gHighContention = new (std::nothrow) SVector<DebugLockNode*>;
      if (gHighContention) {
        gHighContention->setCapacity(gNumContention);
        for (int32_t i = 0; i < gNumContention; i++)
          gHighContention->add(NULL);
      }
      gHighBlockCount = new (std::nothrow) SVector<DebugLockNode*>;
      if (gHighBlockCount) {
        gHighBlockCount->setCapacity(gNumContention);
        for (int32_t i = 0; i < gNumContention; i++)
          gHighBlockCount->add(NULL);
      }
    }
    if (gNumContention > 0)
...
DebugLockNode::DebugLockNode(const char* type, const void* addr,
    ...
    : m_refs(1), m_type(type), m_addr(addr), m_name(name), m_globalFlags(flags), m_createStack(LockDebugLevel() > 5 ? new (std::nothrow) SCallStack : NULL), m_createTime(SysGetRunTime()), m_links(LockDebugLevel() > 10 ? new (std::nothrow) block_links : NULL), m_maxContention(0), m_inContention(false), m_blockCount(0), m_inBlockCount(false)
{
      ...
      if (source && source->m_links) {
        ...
        if (idx < 0) {
          ...
          if (!deadlock) {
            ...
            sptr<ITextOutput> msg          = dirtyADSHack;
            msg << "Thread " << SysCurrentThread()
                << " created a potential deadlock!" << endl
                << endl
                << "While holding " << *source << endl;
            msg->MoveIndent(1);
            source->PrintStacksToStream(msg);
            msg->MoveIndent(-1);
            msg << "attempting to acquire " << *this << endl
                << endl
                << "Previously these were acquired as:" << endl;
            msg->MoveIndent(1);
            const int32_t N = deadlock->size();
            for (int32_t i = 0; i < N; i++) {
...
void DebugLockNode::PrintContentionToStream(const sptr<ITextOutput>& io)
{
  ...
  if (LockContention()) {
    ...
    io << endl
       << "Lock contention summary in process " << SysProcessName() << " (#" << SysProcessID() << "):" << endl
       << endl
       << indent;

    if (gHighContention) {
      const int32_t N = gHighContention->size();
      for (int32_t i = 0; i < N; i++) {
    ...
    if (gHighBlockCount) {
      if (found) io << endl;
      found           = false;
      const int32_t N = gHighBlockCount->size();
      for (int32_t i = 0; i < N; i++) {
...
DebugLock::DebugLock(const char* type, const void* addr,
    ...
    : DebugLockNode(type, addr, name, debug_flags),
      ...
      m_ownerStack(CreationStack() ? new (std::nothrow) SCallStack : NULL),
      m_contention(0),
      m_deleted(false)
{
