...
#define SYSTEM_DIRECTORY "/opt/os"
...
bool SLooper::ProcessManagesContexts(void)
{
  return s_managesContexts;
}

bool SLooper::BecomeContextManager(ContextPermissionCheckFunc checkFunc, void* userData)
{
...
SString get_system_directory()
{
  if (*g_systemDirectory == "") {
    ...
    if (*g_systemDirectory == "") {
      ...
      if (strcmp(g_systemDirectory->PathLeaf(), "os") != 0) {
        char* top = getenv("TOP");
        if (top != NULL) {
    ...
    case brTRANSACTION: {
      ...
      if ((tr.flags & tfRootObject) && (crf = g_catchRootFunc) != NULL) {
        ...
        if (root != NULL)
          ...
          ErrFatalError("Captured transaction does not contain binder!");
      }
...
SLooper::_WaitForCompletion(SParcel* reply, status_t* acquireResult)
{
  ...
  while (1) {
    ...
    if (cmd == brTRANSACTION_COMPLETE) {
      ...
      err = B_BINDER_DEAD;
      if (acquireResult)
        *acquireResult = B_BINDER_DEAD;
      else if (reply)
        reply->Reference(NULL, B_BINDER_DEAD);
      break;
    }
    ...
    else if ((err = _HandleCommand(cmd)))
      break;
  }
...
void SLooper::_BufferFree(const void* data, ssize_t /*len*/, void* context)
{
  if (data) {
#if BINDER_DEBUG_MSGS
