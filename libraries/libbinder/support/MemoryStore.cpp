...
{
  STUB;
  return v;  // STUB: BnStorage::Inspect(caller, v, flags).Join(BByteStream::Inspect(caller, v, flags));
}
...
void BMemoryStore::FreeCore(void *)
{
}
...
void BMallocStore::FreeCore(void *oldBuf)
{
  free(oldBuf);
}
...
BValueStorage::BValueStorage(SValue *value, SLocker *lock, const sptr<SAtom> &object)
    : BMemoryStore((void *)0 /*value->Data()*/, 0 /*value->Length()*/),
      m_value(value),
      m_lock(lock),
      m_atom(object),
      m_editing(false)
