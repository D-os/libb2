...
IDLSymbol::IDLSymbol(const String name)
    : m_id(name), m_type(NULL)
{
}

IDLSymbol::IDLSymbol(const String name, const sp<IDLType>& aType)
...
status_t
IDLSymbol::SetType(const sp<IDLType>& aType)
{	m_type=aType;
    return OK;
}

sp<IDLType> IDLSymbol::GetType()
{	return m_type;
}
