...
StackNode::StackNode(const StackNode& node)  // copy constructor
    : scopename(node.scopename), parentlink(node.parentlink), childlink(node.childlink)
{
...
StackNode::lookup(IDLSymbol sym, bool insert)
{
  ...
  if (insert) {
    if (present) {
      berr << "----- symbolstack.cpp -----> failed to insert " << sym.GetId() << "; symbol already present in" << scopename << endl;
    }
    else {
      scopetable.AddItem(sym.GetId(), sym);
      bout << "----- symbolstack.cpp -----> " << sym.GetId() << " inserted into " << scopename << " table" << endl;
    }
    ...
    if (present) {
      berr << "----- symbolstack.cpp -----> " << sym.GetId() << " symbol present in" << scopename << endl;
      return (&scopetable.EditValueFor(sym.GetId(), &present));
    }
    else {
      scopetable.AddItem(sym.GetId(), sym);
      bout << "----- symbolstack.cpp -----> " << sym.GetId() << " not present " << scopename << " table" << endl;
      return NULL;
    }
  }
}
...
void SymbolStack::push(const SString scope)
{
  StackNode*                       temp;
  StackNode*                       childptr;
  ...
  if (CurrentScope == NULL) {
    ...
    bout << "<----- symbolstack.cpp -----> node is being pushed onto the stack - CurrentScope= " << CurrentScope->getScopeName() << endl;
  }
#endif
  ...
  CurrentScope = temp;
#ifdef SYMDEBUG
  bout << "<----- symbolstack.cpp -----> another node got pushed onto stack - CurrentScope= " << CurrentScope->getScopeName() << endl;
#endif
  //	}
}
