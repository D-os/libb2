...
private:
	...
	SString m_url;
	SString m_urn;
	SVector< sp<NamedType> > m_params;
};

class WsdlClass : public SAtom
{
public:
	WsdlClass(const SString& id, const sp<BWsdl>& wsdl);
	...
	virtual ssize_t AddMethod(const sp<Method>& method);
	virtual ssize_t AddMemeber(const sp<NamedType>& type);
	virtual ssize_t	AddParent(const SString& parent);
	virtual ssize_t AddType(const sp<WsdlType>& type);

	virtual size_t CountMethods() const;
	virtual sp<Method> MethodAt(size_t index) const;

	virtual size_t CountMembers() const;
	virtual sp<NamedType> MemberAt(size_t index) const;

	virtual size_t CountParents() const;
	virtual SString ParentAt(size_t index) const;

	virtual size_t CountTypes() const;
	virtual sp<WsdlType> TypeAt(size_t index) const;
	virtual sp<WsdlType> TypeFor(const SString& id, bool* found = NULL) const;
...
private:
	...
	SVector< sp<Method> > m_methods;
	SVector< sp<NamedType> > m_members;
	SKeyedVector<SString, sp<WsdlType> > m_types;
...
status_t wsdl_create_header(const sp<WsdlClass>& classes, sp<ITextOutput> stream, const SString& filename);
status_t wsdl_create_types_header(const sp<WsdlClass>& classes, sp<ITextOutput> stream, const SString& filename);
status_t wsdl_create_cpp(const sp<WsdlClass>& classes, sp<ITextOutput> stream, const SString& filename, const SString& header);
status_t wsdl_create_types_cpp(const sp<WsdlClass>& classes, sp<ITextOutput> stream, const SString& filename, const SString& header);
status_t wsdl_create_types_interface(const sp<WsdlClass>& classes, sp<ITextOutput> stream);
status_t wsdl_create_interface(const sp<WsdlClass>& classes, sp<ITextOutput> stream, const SString& typesIdl);
