...
const float kScrollerHeight = 12.0f;
...
{
	if (fMenu && fMenu->CountItems() == 0) {
		BRect rect(Bounds());
		be_control_look->DrawMenuBackground(this, rect, updateRect, ui_color(B_MENU_BACKGROUND_COLOR));
...
BMenuWindow::BMenuWindow(const char* name)
	...
	: BWindow(BRect(0, 0, 0, 0), name, B_BORDERED_WINDOW_LOOK, kMenuWindowFeel,
			  B_NOT_MOVABLE | B_NOT_ZOOMABLE | B_NOT_RESIZABLE | B_AVOID_FOCUS | kAcceptKeyboardFocusFlag),
	  fMenu{nullptr},
	  fMenuFrame{nullptr},
	  fUpperScroller{nullptr},
	  fLowerScroller{nullptr},
	  fScrollStep(19)
{
...
void BMenuWindow::AttachScrollers()
{
	...
	BRect frame	   = Bounds();
	float newLimit = fMenu->Bounds().Height() - (frame.Height() - 2 * kScrollerHeight);
	...
	if (!fLowerScroller) {
		fLowerScroller = new LowerScroller(
			BRect(0, frame.bottom - kScrollerHeight + 1, frame.right, frame.bottom));
...
{
	return fMenuFrame && fUpperScroller && fLowerScroller;
}
