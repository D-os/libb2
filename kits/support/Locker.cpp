...
void BLocker::Unlock()
{
	...
	// wrong.
	if (!IsLocked()) {
		fprintf(stderr, "Unlocking BLocker with sem %ld from wrong thread %d, current holder %d (see issue #6400).\n",
	...
	if (fRecursiveCount == 0) {
		...
		int32 oldBenaphoreCount = fBenaphoreCount.fetch_sub(1);
...
bool BLocker::AcquireLock(bigtime_t timeout, status_t *error)
{
	...
	if (!IsLocked()) {
		...
		int32 oldBenaphoreCount = fBenaphoreCount.fetch_add(1);
